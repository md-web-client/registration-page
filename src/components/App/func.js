import { TweenMax, Expo } from 'try-out-a-package-to-be-removed-later'
export const repositionAndPeek = () => {
...
if(false) {
	console.log({passwordLabel, blinking, eyeScale}, stopBlinking)
}
...
function spreadFingers() {
	TweenMax.to(twoFingers, .35, {transformOrigin: "bottom left", rotation: 30, x: -9, y: -2, ease: window.Power2.easeInOut});
}

function closeFingers() {
	TweenMax.to(twoFingers, .35, {transformOrigin: "bottom left", rotation: 0, x: 0, y: 0, ease: window.Power2.easeInOut});
}

function coverEyes() {
	TweenMax.killTweensOf([armL, armR]);
	TweenMax.set([armL, armR], {visibility: "visible"});
	TweenMax.to(armL, .45, {x: -93, y: 10, rotation: 0, ease: window.Quad.easeOut});
	TweenMax.to(armR, .45, {x: -93, y: 10, rotation: 0, ease: window.Quad.easeOut, delay: .1});
	TweenMax.to(bodyBG, .45, {morphSVG: bodyBGchanged, ease: window.Quad.easeOut});
	eyesCovered = true;
}

function uncoverEyes() {
	TweenMax.killTweensOf([armL, armR]);
	TweenMax.to(armL, 1.35, {y: 220, ease: window.Quad.easeOut});
	TweenMax.to(armL, 1.35, {rotation: 105, ease: window.Quad.easeOut, delay: .1});
	TweenMax.to(armR, 1.35, {y: 220, ease: window.Quad.easeOut});
	TweenMax.to(armR, 1.35, {rotation: -105, ease: window.Quad.easeOut, delay: .1, onComplete: function() {
		TweenMax.set([armL, armR], {visibility: "hidden"});
	}});
	TweenMax.to(bodyBG, .45, {morphSVG: bodyBG, ease: window.Quad.easeOut});
	eyesCovered = false;
}
...
function initLoginForm() {
	...
	if(isMobileDevice()) {
		...
		TweenMax.set(twoFingers, {transformOrigin: "bottom left", rotation: 30, x: -9, y: -2, ease: window.Power2.easeInOut});
	}
